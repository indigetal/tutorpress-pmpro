...
class GradeBook {
	...
	public function delete_gradebook() {
		...
		global $wpdb;
		$gradebook_id = Input::get( 'gradebook_id', 0, Input::TYPE_INT );
	...
	public function generate_grade( $submitted_id ) {
		...
		if ( $submitted_info ) {
			...
			if ( ! $gradebook ) {
				tutor_log( 'Gradebook not found for this mark.' );
				return;
			}
	...
	public function gradebook_result_list_bulk_actions() {
		...
		global $wpdb;
		$gradebooks_result_ids = Input::get( 'gradebooks_result_ids', array(), Input::TYPE_ARRAY );
