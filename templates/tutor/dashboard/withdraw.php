...
// TutorPress PMPro: Use fixed withdraw summary method
// On multisite: Fixes {$wpdb->prefix}users bug (should be {$wpdb->users})
// On single-site: Uses Tutor's native method (works fine)
if ( is_multisite() && class_exists( '\TUTORPRESS_PMPRO\Multisite\Withdraw_Summary_Fix' ) ) {
	$summary_data = \TUTORPRESS_PMPRO\Multisite\Withdraw_Summary_Fix::get_withdraw_summary_multisite( $user_id );
	error_log( sprintf( '[TP-PMPRO Withdraw Fix] Using multisite-compatible summary: current_balance=$%s', $summary_data ? $summary_data->current_balance : 'null' ) );
} else {
	$summary_data = WithdrawModel::get_withdraw_summary( $user_id );
	error_log( sprintf( '[TP-PMPRO Withdraw Fix] Using Tutor native summary: current_balance=$%s', $summary_data ? $summary_data->current_balance : 'null' ) );
}
...
// TutorPress PMPro: Format prices with proper decimal places
// Use number_format to ensure 2 decimal places, then wrap with currency symbol
$currency_symbol                  = tutor_utils()->currency_symbol();
$available_for_withdraw_formatted = $currency_symbol . number_format( $available_for_withdraw, 2, '.', ',' );
$current_balance_formated         = $currency_symbol . number_format( $summary_data->current_balance, 2, '.', ',' );
?>
